...
import { createClient, RedisClientType, RedisClusterType, RedisDefaultModules } from 'redis';
import {
  GetOptions,
...
export class RedisService implements OnModuleInit {
  ...
  constructor(
    ...
    this.exitProcessOnConnectionError = this.RedisOptions.exitProcessOnConnectionError || false;
  ...
  public async onModuleInit() {
    await this.init();
    this.logger.log(`\nRedis Connection Checking...\n${await this.sendCommand(['info', 'clients'])}`);
    this.logger.log(`\nRedis Connection Checking...\n${await this.sendCommand(['info', 'server'])}`);
  }

  private async initConfig(client: RedisClientType) {
    const fixedNotifyKeyEventParam = '$xK';
    const nke: any = await client.sendCommand(['config', 'get', 'notify-keyspace-events']);
    if (nke[1] !== fixedNotifyKeyEventParam) {
      await client.sendCommand(['config', 'set', 'notify-keyspace-events', 'K$x']);
    }
  }

  public async createRawConnection(): Promise<RedisClientType> {
    const options = this.redis;
    const client = createClient(options as RedisClientOptions) as RedisClientType;

    client.on('error', (err) => {
      this.isAvailable = false;
      console.error(err.message);
      if (err.message === RETRY_EXCEEDED_ERROR || this.exitProcessOnConnectionError) {
        process.exit(1);
      }
    });
    client.on('connect', () => {
  ...
  private async execute<T>(
    callback: (client: RedisClientType<RedisDefaultModules> | RedisClusterType<RedisDefaultModules>) => Promise<T>
  ): Promise<T> {
    const pool = this.pool;
    const client = await pool.acquire();
    try {
  ...
  public async evalsha(hash: string, evalOptions: EvalOptions) {
    return await this.execute((client) => (client as any).EVALSHA(hash, evalOptions));
  }

  public async eval(script: string, evalOptions: EvalOptions) {
    return await this.execute((client) => (client as any).EVAL(script, evalOptions));
  }
   ...
   */
  public async jsonSet(key: string, path: string, jsonObj: RedisJSON): Promise<'OK' | null> {
    return this.execute((client) => (client as any).json.SET(key, path, jsonObj));
  }
   ...
   */
  public async jsonGet(key: string, getOptions: GetOptions): Promise<RedisJSON | null> {
   ...
   */
  public async jsonObjKeys(key: string, path: string): Promise<Array<string> | null | Array<Array<string> | null>> {
    return this.execute((client) => (client as any).json.OBJKEYS(key, path));
  }

  public async jsonMget(keys: string[], path: string): Promise<Array<RedisJSON | null>> {
  ...
  public async jsonArrlen(key: string, path: string): Promise<number | Array<number>> {
    return this.execute((client) => (client as any).json.ARRLEN(key, path));
  }

  public async jsonArrAppend(key: string, path: string, value: Record<string, any>): Promise<string> {
    return this.execute((client) => (client as any).json.ARRAPPEND(key, path, value));
  }
  ...
  public async xadd(
    ...
    return this.execute((client) => (client as any).XADD(key, id, message, options));
  }

  public async xgroupCreate(key: string, group: string, id: string, mkstream: boolean) {
    return this.execute((client) => (client as any).XGROUP_CREATE(key, group, id, { MKSTREAM: mkstream }));
  }
  ...
  public async xReadGroup(
    ...
    return this.execute((client) => (client as any).XREADGROUP(group, consumer, streams, options));
  }
  ...
  public async xtrimMaxlen(key: string, threshold: number) {
    return this.execute((client) => (client as any).XTRIM(key, 'MAXLEN', threshold));
  }
  ...
  public async xrevrange(key: string, start: string, end: string, options?: { COUNT?: number }): Promise<any[]> {
    return this.execute((client) => (client as any).XREVRANGE(key, start, end, options));
  }
   ...
   * @param key - The list key
   */
  public async blpop(key: string): Promise<{ key: string; element: SingleCommandResult }> {
   ...
   * @param key - The list key
   */
  public async brpop(key: string): Promise<{ key: string; element: SingleCommandResult }> {
   ...
   * @param ttl 만료 시간 (초)
   */
  public async hmsetWithExpire(key: string, hashData: Record<string, any>, ttl: number): Promise<void> {
    const script = `
   ...
   */
  public async hmset(key: string, object: { [field: string]: string }): Promise<number> {
  ...
  public async zrank(key: string, member: string): Promise<number | null | undefined> {
  ...
  public async zincrby(key: string, increment: number, member: string): Promise<number> {
    return this.execute((client) => (client as any).ZINCRBY(key, increment, member));
  }
  ...
  public async zcount(key: string, min: string | number, max: string | number): Promise<number> {
  ...
  public async zrevrange(key: string, start: number, stop: number): Promise<ZMember[]> {
  ...
  public async zrangeByScore(
    ...
    return this.execute((client) => (client as any).ZRANGEBYSCORE_WITHSCORES(key, min, max, zrangeOption));
  }
  ...
  public async zrangeWithscore(key: string, start: string | number, stop: string | number): Promise<ZMember[]> {
    return this.execute((client) => (client as any).ZRANGE_WITHSCORES(key, start, stop));
  }

  public async zrange(key: string, start: string | number, stop: string | number): Promise<string[]> {
    return this.execute((client) => (client as any).ZRANGE(key, start, stop));
  }

  public async zadd(key: string, score: string | number, member: string): Promise<number> {
    return this.execute((client) => (client as any).ZADD(key, { score: +score, value: member }));
  }
  ...
  public async zremrangebyscore(key: string, min: string | number, max: string | number): Promise<number> {
    return this.execute((client) => (client as any).ZREMRANGEBYSCORE(key, min, max));
  }

  public async zrandmember(key: string, count: number): Promise<string[]> {
    return this.execute((client) => (client as any).ZRANDMEMBER_COUNT(key, count));
  }
   ...
   */
  public async set(key: string, data: string | number, ttl = 0): Promise<string | null> {
    return this.execute((client) => (client as any).SET(key, data, ttl ? { EX: ttl } : undefined));
  }
   ...
   * @returns
   */
  public async setMultiple(data: Array<{ key: string; value: string | number; ttl: number }>): Promise<string> {
    const script = `
      for i = 1, #ARGV, 3 do
        local key = ARGV[i]
    ...
    const args = data.flatMap((item) => [item.key, `${item.value}`, `${item.ttl}`]);
    const evalOptions: EvalOptions = {
      arguments: args,
    };
